...
def query(queries, do_filter, delimiter, stdin, stdout):
    ...
    for line in stdin:
        ...
        for query in queries:
            for value, filtered in q(query, copy.deepcopy(json_obj), FilteredJSONObject()):
                if do_filter:
                    stdout.write(json.dumps(filtered) + delimiter)
                else:
                    stdout.write(json.dumps(value) + delimiter)
        if delimiter != '\n':
            print
...
def main():
    ...
    return query(queries, options.do_filter, options.delimiter.decode('string_escape'), sys.stdin, sys.stdout)
